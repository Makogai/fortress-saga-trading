    ...
    return (
      <div className="flex flex-col gap-2 sm:gap-2.5">
        <div className="flex justify-center gap-2 sm:gap-2.5">
          {[0, 1].map((i) => (
            <CountSlot key={`${pageKey}-t-${i}`} card={top[i]} slotIndex={i} />
          ))}
        </div>
        <div className="flex justify-center gap-2 sm:gap-2.5">
          {[0, 1, 2].map((i) => (
...
export function AlbumGrid({
    ...
    <div className="md:hidden space-y-2">
      {cards.map((card, i) =>
        !showCard(card) ? (
          <div
            key={i}
            className="rounded-xl border border-stone-600/30 bg-stone-800/30 min-h-[3.5rem] py-3 px-4 flex items-center justify-center text-stone-500 text-sm"
          >
            {viewMode === 'needOnly' ? '—' : 'Need'}
          </div>
        ) : (
  ...
  return (
    <div
      className={`rounded-xl border border-stone-600/40 bg-stone-800/30 ${
